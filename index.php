...
<div class="container home-page">
    ...
    <?php if($flash_sale_products): ?><section id="flash-sale" class="home-section flash-sale-section"><header class="section-header"><div><p class="eyebrow">LIMITED TIME DEALS</p><h2 class="section-title"><i class="fa-solid fa-bolt"></i> Flash Sale กำลังลดราคา</h2><p>ราคาพิเศษมีผลเฉพาะช่วงเวลาที่ร้านกำหนด</p></div><a href="<?php echo BASE_URL;?>products.php" class="text-link">ดูสินค้าทั้งหมด →</a></header><div class="product-grid compact-grid"><?php foreach($flash_sale_products as $p):?><article class="product-card"><span class="product-badge flash-badge">FLASH SALE</span><a href="<?php echo BASE_URL;?>product-detail.php?id=<?php echo (int)$p['id'];?>" class="product-img-wrapper"><img src="<?php echo e(productImageUrl($p['image_url']));?>" alt="<?php echo e(productDisplayName($p));?>" class="product-img"></a><div class="product-info"><span class="product-category"><?php echo e($p['category_name']);?></span><a class="product-title" href="<?php echo BASE_URL;?>product-detail.php?id=<?php echo (int)$p['id'];?>"><?php echo e(productDisplayName($p));?></a><div class="product-meta"><?php echo productCardPriceHtml($p);?><span class="stock-text in-stock">พร้อมส่ง</span></div><button class="btn btn-cart" onclick="addToCart(<?php echo (int)$p['id'];?>,1)"><i class="fa-solid fa-plus"></i> เพิ่มลงตะกร้า</button></div></article><?php endforeach;?></div></section><?php endif;?>
