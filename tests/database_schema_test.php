...
$columns=[];foreach($db->query("SHOW COLUMNS FROM order_items") as $row)$columns[]=$row['Field'];foreach(['seller_id','variant_id','variant_sku','variant_name'] as $column)if(!in_array($column,$columns,true))throw new RuntimeException('Missing order_items column: '.$column);
...
$sellerColumns=[];foreach($db->query('SHOW COLUMNS FROM seller_profiles') as $row)$sellerColumns[]=$row['Field'];foreach(['store_status','product_submission_enabled','order_processing_enabled','payout_enabled','suspension_reason','suspended_at','suspended_by'] as $column)if(!in_array($column,$sellerColumns,true))throw new RuntimeException('Missing seller_profiles column: '.$column);
