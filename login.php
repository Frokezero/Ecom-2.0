...
function showAuthError(message,field,email,verificationUrl){const box=document.getElementById('authError'),help=document.getElementById('verificationHelp');box.querySelector('span').textContent=message;box.classList.add('show');help.hidden=!email;if(email)help.href=verificationUrl||`${BASE_URL}check-email.php?email=${encodeURIComponent(email)}`;document.querySelectorAll('.auth-field').forEach(el=>el.classList.remove('invalid'));if(field){const input=document.querySelector(`[name="${field}"]`);if(input){input.closest('.auth-field')?.classList.add('invalid');input.focus()}}}
async function refreshLoginToken(){const response=await fetch(`${BASE_URL}login.php`,{credentials:'same-origin',cache:'no-store'});if(!response.ok)throw new Error('เซสชันหมดอายุ กรุณารีเฟรชหน้าแล้วลองใหม่');const page=new DOMParser().parseFromString(await response.text(),'text/html');const token=page.querySelector('#loginForm input[name="csrf_token"]')?.value;if(!token)throw new Error('เซสชันหมดอายุ กรุณารีเฟรชหน้าแล้วลองใหม่');document.querySelector('#loginForm input[name="csrf_token"]').value=token;}
document.getElementById('loginForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,button=document.getElementById('loginBtn'),errorBox=document.getElementById('authError');errorBox.classList.remove('show');document.getElementById('verificationHelp').hidden=true;if(!form.reportValidity())return;button.disabled=true;button.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> กำลังตรวจสอบ';try{let response=await fetch(`${BASE_URL}api/auth.php`,{method:'POST',credentials:'same-origin',body:new FormData(form)});let result=await response.json();if(response.status===403&&result.message?.includes('โทเคนความปลอดภัยไม่ถูกต้อง')){await refreshLoginToken();response=await fetch(`${BASE_URL}api/auth.php`,{method:'POST',credentials:'same-origin',body:new FormData(form)});result=await response.json();}if(!response.ok||result.status!=='success')throw Object.assign(new Error(result.message||'ไม่สามารถเข้าสู่ระบบได้'),{field:result.data?.field,retryAfter:result.data?.retry_after,email:result.data?.verification_required?result.data?.email:null,verificationUrl:result.data?.verification_url});showToast(result.message,'success');setTimeout(()=>location.href=result.data.redirect,350)}catch(error){showAuthError(error.message,error.field,error.email,error.verificationUrl);button.disabled=false;button.innerHTML='<span>เข้าสู่ระบบ</span><i class="fa-solid fa-arrow-right"></i>'}});
</script>
