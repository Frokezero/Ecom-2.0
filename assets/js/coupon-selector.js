(function(){
  ...
  scope.querySelectorAll('[data-coupon-selector]').forEach(root=>{
    ...
    const reset=()=>{if(total)total.textContent=money(subtotal);syncPayable(subtotal);if(discount)discount.textContent='-฿0.00';if(discountRow)discountRow.style.display='none';};
    ...
    const useCode=async code=>{code=String(code||'').trim();if(!code)return clearCoupon();message.textContent='กำลังตรวจสอบคูปอง...';const {response,result}=await post({action:'validate',code});if(response.status===401){message.textContent='เข้าสู่ระบบเพื่อใช้คูปอง';message.style.color='#b83b32';return;}if(result.status!=='success'){message.textContent=result.message||'ใช้คูปองไม่ได้';message.style.color='#b83b32';reset();return;}input.value=result.data.code;select.value=result.data.code;message.textContent=`เลือกใช้ ${result.data.code} แล้ว`;message.style.color='#28704a';if(discount)discount.textContent=`-${money(result.data.discount)}`;if(discountRow)discountRow.style.display='flex';if(total)total.textContent=money(result.data.total);syncPayable(result.data.total);};
    const clearCoupon=async()=>{await post({action:'clear'}).catch(()=>{});select.value='';input.value='';message.textContent='คำสั่งซื้อนี้จะไม่ใช้คูปอง';message.style.color='var(--muted)';reset();};
    select.addEventListener('change',()=>select.value?useCode(select.value):clearCoupon());
    applyButton.addEventListener('click',()=>input.value.trim()?useCode(input.value):clearCoupon());
