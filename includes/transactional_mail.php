...
function processQueuedEmail(PDO $db,string $dedupeKey):bool{
    $db->beginTransaction();
    try{$stmt=$db->prepare("SELECT * FROM email_delivery_logs WHERE dedupe_key=? AND status IN ('queued','failed') AND (next_attempt_at IS NULL OR next_attempt_at<=NOW()) FOR UPDATE");$stmt->execute([$dedupeKey]);$mail=$stmt->fetch();if(!$mail){$db->rollBack();return true;}$db->prepare("UPDATE email_delivery_logs SET status='sending',locked_at=NOW() WHERE id=?")->execute([(int)$mail['id']]);$db->commit();}
