...
function productCardPriceHtml(array $product): string {
    ...
    $percent=max(1,min(99,(int)round((1-$current/$regular)*100)));
    return '<span class="card-sale-price"><strong class="product-price">'.formatCurrency($current).'</strong><span><del>'.formatCurrency($regular).'</del><mark>-'.$percent.'%</mark></span></span>';
}
...
function createNotification(PDO $db, int $userId, string $type, string $title, string $body = '', ?string $link = null, bool $sendImmediately = false): void {
    if ($userId < 1) return;
    try{$stmt = $db->prepare('INSERT INTO notifications (user_id,type,title,body,link) VALUES (?,?,?,?,?)');
    ...
    }}catch(Throwable $error){error_log('Notification failed ['.$type.']: '.$error->getMessage());}
}
