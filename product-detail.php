...
<div class="container product-detail-page">
    ...
    <div class="product-detail-shell">
        <div class="product-gallery">
            <?php $primaryVideo=($product['primary_media_type']??'image')==='video'&&!empty($product['video_url']);if(!empty($product['video_url'])&&!$primaryVideo):?><a class="product-video-link" href="<?php echo e($product['video_url']);?>" target="_blank" rel="noopener"><i class="fa-solid fa-circle-play"></i> ดูวิดีโอสินค้า</a><?php endif;?>
            <?php if ((int)$product['is_featured'] === 1): ?><span class="product-badge badge-soft">สินค้าแนะนำ</span><?php endif; ?>
            <?php if(!empty($product['video_url'])):?><video id="mainProductVideo" class="product-main-video" src="<?php echo e(str_starts_with($product['video_url'],'assets/')?BASE_URL.$product['video_url']:$product['video_url']);?>" autoplay muted loop playsinline controls <?php echo $primaryVideo?'':'hidden';?>></video><?php endif;?><img id="mainProductImage" src="<?php echo e(productImageUrl($product['image_url'])); ?>" alt="<?php echo e($product['name']); ?>" <?php echo $primaryVideo?'hidden':'';?>><?php if($product_images||!empty($product['video_url'])):?><div class="product-media-thumbs" data-product-gallery><?php if(!empty($product['video_url'])):?><button type="button" data-product-media="video" aria-label="แสดงวิดีโอ"><i class="fa-solid fa-play"></i></button><?php endif;?><?php foreach(array_merge([['image_url'=>$product['image_url']]],$product_images) as $image):?><button type="button" data-product-media="image" data-src="<?php echo e(productImageUrl($image['image_url']));?>"><img src="<?php echo e(productImageUrl($image['image_url']));?>" alt="" style="width:56px;height:56px;object-fit:cover"></button><?php endforeach;?></div><?php endif;?>
            ...
            <div class="checkout-card">
                <?php if(isLoggedIn() && $can_review): ?>
                    ...
                    <form id="reviewForm" onsubmit="saveReview(event)">
                        ...
                        <label for="reviewComment" style="display:block;font-weight:600;margin-bottom:7px">ความคิดเห็น</label><textarea id="reviewComment" name="comment" minlength="5" maxlength="1000" rows="5" required style="width:100%;padding:11px;margin-bottom:14px" placeholder="สินค้านี้เป็นอย่างไรบ้าง"><?php echo e($current_review['comment']??''); ?></textarea>
...
function showProductImage(src){const image=document.getElementById('mainProductImage'),video=document.getElementById('mainProductVideo');if(!image)return;if(src)image.src=src;image.hidden=false;if(video){video.pause();video.hidden=true}}function showProductVideo(){const image=document.getElementById('mainProductImage'),video=document.getElementById('mainProductVideo');if(!video)return;if(image)image.hidden=true;video.hidden=false;video.play().catch(()=>{})}function addSelectedVariant(){const select=document.getElementById('variantSelect');if(select&&!select.value){showToast('กรุณาเลือกตัวเลือกสินค้า','error');return}addToCart(<?php echo (int)$product['id'];?>,parseInt(document.getElementById('detailQty').value),select?parseInt(select.value):0)}function updateVariant(){const option=document.getElementById('variantSelect')?.selectedOptions[0];if(!option?.value)return;document.querySelector('.detail-price strong').textContent=new Intl.NumberFormat('th-TH',{style:'currency',currency:'THB'}).format(option.dataset.price);document.getElementById('detailQty').max=option.dataset.stock;if(option.dataset.image)showProductImage(option.dataset.image);}
