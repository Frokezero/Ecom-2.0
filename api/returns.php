...
try{
 ...
 $order=$db->prepare("SELECT id,order_no,COALESCE(delivered_at,created_at) delivered_at FROM orders WHERE id=? AND user_id=? AND order_status IN ('shipped','completed') FOR UPDATE");$order->execute([$orderId,$uid]);$row=$order->fetch();if(!$row)throw new RuntimeException('คำสั่งซื้อนี้ยังขอคืนสินค้าไม่ได้');if(strtotime($row['delivered_at'])<strtotime('-7 days'))throw new RuntimeException('พ้นระยะเวลาคืนสินค้า 7 วันแล้ว');
 $exists=$db->prepare("SELECT id FROM return_requests WHERE order_id=? AND status NOT IN ('rejected','cancelled') LIMIT 1");$exists->execute([$orderId]);if($exists->fetch())throw new RuntimeException('คำสั่งซื้อนี้มีคำขอคืนสินค้าแล้ว');
 ...
 if(!$selected)throw new RuntimeException('เลือกสินค้าที่ต้องการคืนอย่างน้อย 1 รายการ');createRoleNotification($db,'admin','return','มีคำขอคืนสินค้าใหม่','คำสั่งซื้อ '.$row['order_no'].' รอตรวจสอบ',BASE_URL.'admin/returns.php');$db->commit();jsonResponse('success','ส่งคำขอคืนสินค้าแล้ว',['redirect'=>BASE_URL.'my-returns.php']);
