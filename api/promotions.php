...
if($action==='validate'){
    ...
    $code=(string)($_POST['code']??$_GET['code']??'');$single=calculateCouponDiscount($db,$code,(int)$_SESSION['user_id'],$items,$subtotal);if($single['error']!=='')jsonResponse('error',$single['error'],[],422);$slot=$single['coupon']['discount_type']==='free_shipping'?'shipping':'product';$requestedSlot=(string)($_POST['slot']??'');if($requestedSlot!==''&&$requestedSlot!==$slot)jsonResponse('error',$requestedSlot==='shipping'?'รหัสนี้ไม่ใช่คูปองส่วนลดค่าจัดส่ง':'รหัสนี้ไม่ใช่คูปองส่วนลดสินค้า',[],422);$selected=$_SESSION['coupon_codes']??[];$selected[$slot]=$single['coupon']['code'];$result=calculateCouponSet($db,array_values($selected),(int)$_SESSION['user_id'],$items,$subtotal);if($result['error']!=='')jsonResponse('error',$result['error'],[],422);$_SESSION['coupon_codes']=$selected;unset($_SESSION['coupon_code']);$charges=$result['charges'];jsonResponse('success','ใช้คูปองได้',['discount'=>$result['discount'],'code'=>$single['coupon']['code'],'slot'=>$slot,'selected_codes'=>array_values($selected),'selected_slots'=>$selected,'subtotal'=>$subtotal,'shipping_amount'=>$charges['shipping'],'shipping_discount_amount'=>$charges['shipping_discount'],'platform_shipping_subsidy'=>$charges['platform_shipping_subsidy'],'seller_shipping_subsidy'=>$charges['seller_shipping_subsidy'],'tax_amount'=>$charges['tax'],'vat_rate'=>$charges['vat_rate'],'shipping_lines'=>$charges['shipping_lines'],'total'=>$charges['total']]);
}
if($action==='clear'){if($_SERVER['REQUEST_METHOD']!=='POST')jsonResponse('error','อนุญาตเฉพาะ POST',[],405);requireCsrf();$slot=(string)($_POST['slot']??'');$selected=$_SESSION['coupon_codes']??[];if(in_array($slot,['product','shipping'],true))unset($selected[$slot]);else $selected=[];$_SESSION['coupon_codes']=$selected;unset($_SESSION['coupon_code']);$items=$_SESSION['cart']??[];$subtotal=0;foreach($items as $item)$subtotal+=(float)$item['price']*(int)$item['quantity'];$result=calculateCouponSet($db,array_values($selected),(int)$_SESSION['user_id'],$items,$subtotal);$charges=$result['charges'];jsonResponse('success','ยกเลิกคูปองแล้ว',['selected_slots'=>$selected,'discount'=>$result['discount'],'shipping_amount'=>$charges['shipping'],'shipping_discount_amount'=>$charges['shipping_discount'],'tax_amount'=>$charges['tax'],'total'=>$charges['total']]);}
if($action==='mine'){$stmt=$db->prepare('SELECT c.id,c.code,c.title,c.description,c.discount_type,c.discount_value,c.max_discount,c.min_order_amount,c.per_user_limit,c.ends_at,uc.used_count FROM user_coupons uc JOIN coupons c ON c.id=uc.coupon_id WHERE uc.user_id=? AND c.is_active=1 AND c.starts_at<=NOW() AND c.ends_at>=NOW() AND (c.per_user_limit=0 OR uc.used_count<c.per_user_limit) ORDER BY c.ends_at');$stmt->execute([(int)$_SESSION['user_id']]);$selected=$_SESSION['coupon_codes']??[];jsonResponse('success','โหลดคูปองสำเร็จ',['coupons'=>$stmt->fetchAll(),'selected_codes'=>array_values($selected),'selected_slots'=>$selected]);}
