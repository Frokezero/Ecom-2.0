...
if ($action === 'login') {
    ...
    $notifyIdentityThreshold=function(int $failures)use($db,$identityHash,$ipFailures,$window):void{try{$marker='%"identity_hash":"'.$identityHash.'"%';$seen=$db->prepare("SELECT id FROM security_events WHERE event_type='login.threshold' AND created_at>DATE_SUB(NOW(),INTERVAL ? SECOND) AND metadata_json LIKE ? LIMIT 1");$seen->execute([$window,$marker]);if(!$seen->fetchColumn()){recordSecurityEvent($db,'login.threshold',0,null,['identity_hash'=>$identityHash,'identity_failures'=>$failures,'ip_failures'=>$ipFailures],'identity_blocked');createRoleNotification($db,'admin','security','แจ้งเตือน: ล็อกอินผิดครบ 10 ครั้ง','ชื่อผู้ใช้หนึ่งถูกลองรหัสผ่านผิดซ้ำภายใน 15 นาที ระบบพักการเข้าสู่ชื่อผู้ใช้นี้ชั่วคราว โดยบัญชีอื่นยังเข้าได้ตามปกติ',BASE_URL.'admin/security-center.php');}}catch(Throwable $ignored){}};
    ...
    if($identityFailures>=$identityRule['threshold_count']){$notifyIdentityThreshold($identityFailures);jsonResponse('error','ชื่อผู้ใช้นี้กรอกรหัสผิดครบ 10 ครั้ง กรุณารอ 15 นาทีหรือใช้เมนูลืมรหัสผ่าน บัญชีอื่นยังเข้าสู่ระบบได้ตามปกติ',['retry_after'=>$identityRule['block_seconds'],'failed_attempts'=>$identityFailures,'block_scope'=>'identity'],429);}
    ...
    if (!$user || !password_verify($password,$user['password_hash'])) {$db->prepare('INSERT INTO login_attempts(identifier_hash,ip_hash,was_successful) VALUES(?,?,0)')->execute([$identityHash,$ipHash]);$attemptMap[$identityHash]=['count'=>(int)$attempts['count']+1,'last'=>time()];$_SESSION['login_attempts']=$attemptMap;recordSecurityEvent($db,'login.failed',10,$user?(int)$user['id']:null,['identifier_hash'=>$identityHash,'attempt'=>$identityFailures+1],'monitored');if($identityFailures+1>=$identityRule['threshold_count']){$notifyIdentityThreshold($identityFailures+1);jsonResponse('error','ชื่อผู้ใช้นี้กรอกรหัสผิดครบ 10 ครั้ง ระบบพักการเข้าสู่ชื่อนี้ 15 นาทีและแจ้งผู้ดูแลแล้ว',['retry_after'=>$identityRule['block_seconds'],'failed_attempts'=>$identityFailures+1,'block_scope'=>'identity'],429);}jsonResponse('error','ชื่อผู้ใช้ อีเมล หรือรหัสผ่านไม่ถูกต้อง',[],401);}
