...
if ($action === 'login') {
    ...
    $known=$db->prepare('SELECT COUNT(*) FROM user_login_locations WHERE user_id=?');$known->execute([(int)$user['id']]);$hasHistory=(int)$known->fetchColumn()>0;$location=$db->prepare('SELECT id FROM user_login_locations WHERE user_id=? AND ip_hash=?');$location->execute([(int)$user['id'],$ipHash]);$knownHere=(bool)$location->fetchColumn();$country=appCountryCode();if($hasHistory&&!$knownHere){recordSecurityEvent($db,'login.new_ip',20,(int)$user['id'],['country_code'=>$country],'verification_recommended');createNotification($db,(int)$user['id'],'security','พบการเข้าสู่ระบบจาก IP ใหม่','บัญชีของคุณเข้าสู่ระบบจากเครือข่ายใหม่ หากไม่ใช่คุณกรุณาเปลี่ยนรหัสผ่านทันที',BASE_URL.'profile.php');}$agentHash=hash('sha256',(string)($_SERVER['HTTP_USER_AGENT']??''));$remember=$db->prepare('INSERT INTO user_login_locations(user_id,ip_hash,country_code,user_agent_hash) VALUES(?,?,?,?) ON DUPLICATE KEY UPDATE login_count=login_count+1,last_seen_at=NOW(),country_code=VALUES(country_code),user_agent_hash=VALUES(user_agent_hash)');$remember->execute([(int)$user['id'],$ipHash,$country,$agentHash]);
