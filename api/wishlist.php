...
if($_SERVER['REQUEST_METHOD']!=='POST')jsonResponse('error','อนุญาตเฉพาะ POST',[],405);if(!isLoggedIn())jsonResponse('error','กรุณาเข้าสู่ระบบก่อน',[],401);if(($_SESSION['user_role']??'')!=='customer')jsonResponse('error','รายการโปรดใช้ได้เฉพาะบัญชีผู้ซื้อ',[],403);requireCsrf();
